...
//sensor
	...
	if (iRet == 0)
	{
		sprintf(_query, QUERY_INSERT_SETTING);
...
void *SensorInterruptLoop(void *vp)
{
	...
	while(1)
	{
		...
		_human_sensor = get_human_sensor();
		_ultra_sensor = get_ultra_sensor();

		sprintf(_query,QUERY_INSERT_SENSOR_DATA, _human_sensor, _ultra_sensor);
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
		sprintf(_query,QUERY_INSERT_SENSOR_CHECK, _human_sensor, _ultra_sensor);
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
		sprintf(_query,QUERY_INSERT_ACTUATOR_VALUE, _use_dcmotor, _use_rgbled, _use_ledbar);
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
		sprintf(_query,QUERY_INSERT_ACTUATOR_CHECK, get_dcmotor_functionality(), get_rgbled_functionality(), get_light_functionality());
		printf("%s : write to DB - %s\n", __func__, _query);
		iot_insert_data_from_db(_query);
	...
	while (1)
	{
		...
		case TOY:
			if (_message->_command == GET_STATUS)
			{
				...
				iRet = sprintf(ucSBuff, "%d|%d|%d,%d,%d,%d,%d", _message->_apptype, _message->_command, _human_sensor, _ultra_sensor, get_dcmotor_functionality(), get_rgbled_functionality(), get_light_functionality());
			...
			else if (_message->_command == GET_SETTING)
			{
				...
				iRet = sprintf(ucSBuff, "%d|%d|%d,%d,%d,%d", _message->_apptype, _message->_command, _result, _use_dcmotor, _use_rgbled, _use_ledbar);
				free(_result);
				write(stMyInfo.iSock, ucSBuff, iRet);
			}
			...
			else if (_message->_command == TOY_DCMOTOR)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
			...
			else if (_message->_command == TOY_RGBLED)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
			...
			else if (_message->_command == TOY_LEDBAR)
			{
				memset(_data, 0x0, sizeof(Data));
				iRet = parse_data(_message->_data, _data);
				_use_ledbar = _data->_data1;
				set_light_active(_data->_data1);
				iRet = sprintf(ucSBuff, "%d|%d", _message->_apptype, _message->_command);
				write(stMyInfo.iSock, ucSBuff, iRet);
			}
			break;
